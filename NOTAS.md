...
- Bloque 3, cambio 2/2: añadidas tarjetas de **Jimbaran** (`id="zona-jimbaran"`) y **Nusa Penida** (`id="zona-nusapenida"`) al final de "Zonas de Bali" (antes de la tarjeta del sarong/templo). Ninguna de las dos ha sido visitada por nosotros — llevan la etiqueta "Info no verificada por nosotros" y un `tip-alert` explícito con el disclaimer, mismo criterio que MyBlueBird en Lombok. Añadidas también al desplegable "Lugares".
- Estado: subido a GitHub (`origin/main`).
...
### 2026-09-17 — Sulawesi (Bloque 3, punto 5: sección "Todos los sitios de la ruta")
- **Fotos conectadas de verdad:** de las 9 tarjetas de lugar, 6 tenían foto real ya subida al repo pero nunca se mostraban (el HTML solo dibujaba un placeholder fijo con icono + nombre de archivo, nunca un `<img>` real, pese a que el texto de ayuda decía "aparecerán automáticamente"). Ahora esas 6 usan `<img class="lugar-foto" src="...">`: Paisu Pok, Pantai Poganda, Kawalu Bay, Long Beach, Tanjung Gundul, Teluk Lalong.
- **Bunaken sin tocar, a propósito:** `bunaken-arrecife.jpg` no muestra un arrecife (es una foto de un atardecer en un muelle) — se deja como placeholder hasta que el usuario confirme si hay otra foto o si se ajusta el texto de la tarjeta.
- **Fotos que faltan (Corner Beach Salakan, Pantai Bontolan):** sin archivo todavía. Se optó por un placeholder claro "Foto próximamente" en vez de rellenar con la foto de otro sitio (`long-beach-banggai-2.jpg`, que existe en el repo pero no está en uso) — mostrar una foto de un lugar distinto contradiría la propia frase de la sección ("cada foto corresponde al sitio exacto"), el mismo problema detectado en Bunaken.
- **Encuadre corregido con `object-position` por CSS** en las fotos donde se veía demasiado cielo/roca y poco sujeto: Long Beach (bajado a `center 85%`), Tanjung Gundul (`center 80%`, para priorizar el arco y las personas sobre la roca vacía de arriba), Kawalu Bay y Teluk Lalong (`center 60%`), Pantai Poganda (`center 60%`), Paisu Pok (`center 55%`). Todas se pudieron arreglar solo con CSS, sin necesidad de recortar o sustituir el archivo.
- **Sección movida:** de estar penúltima en la página (entre "Dinero" y "Agua") pasa a ir justo después de "Por qué ir" — la segunda sección de la página, para que destaque desde el principio, según lo pedido. Sigue sin tener botón propio en el `cat-nav` superior (solo el diagnóstico lo señaló; no se ha añadido — no se pidió explícitamente).
- **Bug de "Tana Toraja" arreglado:** la tarjeta de Tana Toraja (dentro de `id="destinos"`) tiene ahora `id="tana-toraja"`, y el enlace del desplegable "🏝️ Tana Toraja" apunta ahí en vez de a la sección de fotos.
- Nota de la sección actualizada: ya no dice que las fotos "aparecen automáticamente" (era falso, no había código que lo hiciera) — ahora explica que Corner Beach y Pantai Bontolan están pendientes de subir su foto.
